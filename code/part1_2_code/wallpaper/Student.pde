...
{
  //System.out.println("mirror() is called");
  //System.out.format("P in is %f, %f \n", P.x, P.y);
  vec PO = V(P, reflection_axis_pt);
  vec n = U(reflection_axis_vec);
  vec pToV = M(PO, V(dot(PO, n), n));
  return P(P, V(2, pToV));
}
...
{
  return Translate(Mirror(P,glide_axis_pt, glide_axis_vec),U(glide_axis_vec).mul(dist));
}
...
void setWallpaperSymmetries()
{
  ...
  switch(WP.wallpaper_id) {
    ...
    case 3:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("GLIDE", O, U, n(U)/2)); 
      break;
    case 4:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("MIRROR", P(O, U), M(U, V)));
      break;
    case 5:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
    ...
    case 7:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", P(O, V(0.5, U)), PI));
      WP.addSymmetry(new symmetry("GLIDE", P(O, V(0.25, V)), U, n(U)/2));
      break;
    case 8:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", P(O,V(0.5, U)), PI)); 
      WP.addSymmetry(new symmetry("MIRROR", O, V(0.5, A(U, V))));
      break;
    
    case 9:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", P(P(O, V(0.5, V)), V(0.5, U)), PI*0.5));
      break;
     case 10:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
    ...
    case 11:
      ...
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", P(P(O, V(0.5, V)), V(0.5, U)), PI*0.5));
      WP.addSymmetry(new symmetry("GLIDE", P(O, V(0.5, V)), V(0.5, A(U, V)), 0));
      break;
    case 12:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", O, PI*2/3));
      break;
    case 13:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
    ...
    case 15:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
      WP.addSymmetry(new symmetry("ROTATE", O, PI/3));
      break;
    case 16:
      WP.addSymmetry(new symmetry("TRANSLATE", U));
      WP.addSymmetry(new symmetry("TRANSLATE", V));
...
{
  if (myGif) {
    System.out.println("myGif is true");
    strokeWeight(1);
    int max_frames = 90; 
    if(frame>max_frames) frame = 0;
    float t = sq(cos(TWO_PI*frame/max_frames));
    
    if (!animatedF) drawMyPattern(90); 
    else drawMyPattern(60 * t); 
    return;
  }
  if (showF) {
    ...
    if (animatedF) ellipse(2.*a/5, -a/24, t*a/4, a/30);
    else ellipse(2.*a/5, -a/24, a/4, a/30);
...
float theta; 
void drawMyPattern(float a) {
  stroke(blue);
  
  //float a = (mouseX / (float) width) * 90f;
  theta = radians(a);
  translate(2.*a/5, -a/24);
  line(0,0,0,-60);
  translate(0,-60);
  branch(120);
}
...
void branch(float h) {
  ...
  if (h > 2) {
    ...
    // branch off to the "bottom right"
    pushMatrix();
    rotate(theta + 90);
    line(0, 0, 0, -h);
    translate(0, -h);
