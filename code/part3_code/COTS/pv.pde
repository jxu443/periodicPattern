...
class pt 
  ...
  pt moveWithMouseMy() { x += mouseX-pmouseX; y -= mouseY-pmouseY;  return this;}; 
  pt moveWithMouseMx() { x -= mouseX-pmouseX; y += mouseY-pmouseY;  return this;}; 
...
// Point in frame of another
pt M(pt A, pt B) {return P((A.x-B.x),(A.y-B.y)); };                                          // (A+B)/2 (average)
...
// display 
void show(float x, float y, float r) {ellipse(x, y, 2*r, 2*r);};                                             // draws circle of center r around P
...
void label(pt P, String S) {text(S, P.x-1.,P.y-2.5); }                                                 // writes string S next to P on the screen ( for example label(P[i],str(i));)
void label(pt P, vec V, String S) {text(S, P.x-3.5+V.x,P.y+7+V.y); }                                  // writes string S at P+V
void showId(pt P, String S) {fill(white); show(P,16); fill(black); label(P,S);}                       // sows disk with S written inside
void showId(pt P, String S, float r, vec V) {fill(white); show(P,r); fill(black); label(P,V,S);}                       // sows disk with S written inside
...
float toDeg(float a) {return a*180/PI;}                                                           // convert radians to degrees
